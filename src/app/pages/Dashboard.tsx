...
export function Dashboard() {
  ...
  // ✅ الرجوع للرئيسية بدون مسح الـ role — Context يحافظ عليه
  const handleGoHome = () => {
    navigate('/');
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20 text-right" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[3rem] shadow-sm border border-stone-100 gap-6">
        ...
        <div className="flex gap-3">
          <Button onClick={handleGoHome} variant="outline" className="rounded-2xl font-black h-12 border-stone-200">
            <Home className="ml-2" size={18} /> الرئيسية
          </Button>
          <Button onClick={handleLogout} className="rounded-2xl font-black h-12 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white border-none shadow-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 border-none shadow-sm rounded-[3rem] bg-white p-4">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button onClick={() => navigate('/dashboard/projects')} variant="ghost" className="text-vox-primary font-black hover:bg-vox-light">
              كل المشاريع ←
            </Button>
          </CardHeader>
